...
UPD_RAW_URL="${HEROBOARD_UPDATE_URL:-https://raw.githubusercontent.com/rnm-dev/claude-plugins/main/plugins/heroboard/.claude-plugin/plugin.json}"
...
hb_update_nudge() {
  ...
  if hb_version_gt "$latest" "$installed"; then
    ...
    printf '{"systemMessage":"Heroboard: plugin update available (%s → %s). Run  /plugin update heroboard@rnm-plugins  then  /reload-plugins  to upgrade."}\n' "$installed" "$latest"
  fi
}
